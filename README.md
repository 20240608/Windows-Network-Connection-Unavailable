...
---
![网络连接不可用]（‪C:\Users\HP\OneDrive\Pictures\Screenshots\屏幕截图 2025-04-06 182218.png）
![未识别的网络](C:\Users\HP\OneDrive\Pictures\Screenshots\屏幕截图 2025-04-09 104456.png)
...
2. 找到你的 **“以太网”**<br>（如果是有线网络是这块网卡，如果是无线网络则需要找其他网卡，总之，结合你的实际情况，找到你 **“当前使用的网卡”**），<br>右键选择 **“属性”**。
3. 选中 **“Internet 协议版本 4（TCP/IPv4）”**，然后点击 **“属性”**。
4. 选择 **“自动获取 IP 地址”** 和 **“自动获取 DNS 服务器地址”**，试试能否运行。
5. >如果步骤4不能运行，选择 **“使用以下 IP 地址”**，输入学校提供的 IP 信息<br>（如果没有，可以**联系校园网管理员或用下面的方法获取**）。
   >>快速查看 **“获取校园网/内网当前可用的网络配置的方法”（用其他手机或电脑）**
   >>>手机或电脑连接校园网/内网 WiFi，或用网线连接其他电脑；
   >>>>手机：打开“设置” → “WiFi” → 选择已连接网络 → 找到网络配置信息,  **“ip、默认网关、子网掩码、DNS”** 等；<br>电脑：从 步骤1-3的 **“Internet 协议版本 4（TCP/IPv4）”**，点击 **“属性”**，找到网络配置信息, **“ip、默认网关、子网掩码、DNS”** 等；
   >>>>>把这里的信息填到电脑的 **“Internet 协议版本 4（TCP/IPv4）”** ， **“属性”** 里。
   >>>>>>抄 **“IP地址”** 时注意，末尾稍微加几个数，避免 **“IP地址”** 冲突。<br>例如： **“IP 地址：58.555.249.217（你也可以改成 58.555.249.218、219 避免冲突）”**
6. 点击 **确定**，然后测试网络连接。
...
- **校园网/内网 DHCP 服务器故障**（无法分配 IP）。

你可以联系学校/企业的 IT 部门或网管，提供 **电脑的 MAC 地址**（在 cmd 里输入 `ipconfig /all`，找到 **“物理地址”**），让他们检查是否有网络限制。
